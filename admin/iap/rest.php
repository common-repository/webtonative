...
class WebtonativeIAPRestController
{
  public $path = 'webtonative/iap';
  ...
  {
    global $webtonativeIAP;
    register_rest_route($this->path, '/product', array(
      'methods' => 'POST',
      'callback' => array($this, 'get_native_id'),
    ));
    register_rest_route($this->path, '/order', array(
      'methods' => 'POST',
  ...
  function get_native_id(WP_REST_Request $request)
  {
    ...
    if (!isset($request_body['productId']) || !isset($request_body['platform'])) {
  ...
  function create_woocommerce_order(WP_REST_Request $request)
  {
    ...
    if (!isset($request_body['productId']) || !isset($request_body['platform']) || !isset($request_body['nativeProductId'])) {
      return new WP_REST_Response(array('error' => 'Invalid Request'), 400);
    }
